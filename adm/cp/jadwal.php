			    ...
			    if ($i > 1) 
			    	{
				      ...
				      $o_tingkat = cegah($sheet['F']);
						...
						//cek
						$qcc = mysqli_query($koneksi, "SELECT * FROM m_jadwal ".
												"WHERE waktu = '$o_waktu' ".
												"AND pukul = '$o_pukul' ".
												"AND tingkat = '$o_tingkat'");
						...
						if (!empty($tcc))
							{
							mysqli_query($koneksi, "UPDATE m_jadwal SET waktu = '$o_waktu', ".
											"durasi = '$o_durasi', ".
											"mapel = '$o_mapel', ".
											"tingkat = '$o_tingkat' ".
											"WHERE no = '$o_no' ".
							...
							{
							//insert
							mysqli_query($koneksi, "INSERT INTO m_jadwal(kd, no, waktu, pukul, durasi, mapel, tingkat, postdate) VALUES ".
...
if ($_POST['btnEX'])
	...
	//data
	$qdt = mysqli_query($koneksi, "SELECT * FROM m_jadwal ".
...
if ($_POST['btnSMP'])
	...
	if ((empty($e_no)) OR (empty($e_waktu)) OR (empty($e_pukul)) OR (empty($e_durasi)) OR (empty($e_mapel)) OR (empty($e_tingkat)))
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_jadwal SET no = '$e_no', ".
							"waktu = '$e_waktu', ".
							"pukul = '$e_pukul', ".
							"durasi = '$e_durasi', ".
							"mapel = '$e_mapel', ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_jadwal ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_jadwal ".
						"WHERE kd = '$kdx'");
