...
//nilai
$filenya = "log.php";
$judul = "[SETTING] Data Pembuat Soal";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($_POST['btnSMP'])
	...
	//nek null
	if ((empty($e_user)) OR (empty($e_nama)))
		{
		//re-direct
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO adminx(kd, usernamex, passwordx, nama, postdate) VALUES ".
							"('$e_kd', '$e_user', '$e_pass', '$e_nama', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE adminx SET usernamex = '$e_user', ".
							"passwordx = '$e_pass', ".
							"nama = '$e_nama', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM adminx ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysqli_query($koneksi, "SELECT * FROM adminx ".
						"WHERE kd = '$kd'");
	...
	<input name="e_user" type="text" size="20" value="'.$e_user.'" class="btn-warning">
	</p>
	...
	echo '<div class="table-responsive">          
		  <table class="table" border="1">
		    <thead>
				...
				<td><strong><font color="'.$warnatext.'">LAST LOGIN</font></strong></td>
				</tr>
