			    ...
			    if ($i > 1) 
			    	{
				      ...
				      $i_kunci = cegah($sheet['C']);
						...
						if (!empty($tcc))
							{
							mysqli_query($koneksi, "UPDATE m_soal SET isi = '$i_isi', ".
											"kunci = '$i_kunci', ".
											"postdate = '$today' ".
											"WHERE jadwal_kd = '$skd' ".
											"AND no = '$i_no'");
							...
							{
							//insert
							mysqli_query($koneksi, "INSERT INTO m_soal(kd, jadwal_kd, no, isi, kunci, postdate) VALUES ".
...
if ($_POST['btnSMP'])
	...
	if ((empty($e_no)) OR (empty($editor)) OR (empty($e_kunci)))
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_soal SET no = '$e_no', ".
							"isi = '$editor', ".
							"kunci = '$e_kunci', ".
							"postdate = '$today' ".
							"WHERE jadwal_kd = '$skd' ".
...
*/
	...
	//ketahui jumlah soal
	$qmboh = mysqli_query($koneksi, "SELECT * FROM m_soal ".
							"WHERE jadwal_kd = '$skd'");
	$tmboh = mysqli_num_rows($qmboh);
	
	//update
	mysqli_query($koneksi, "UPDATE m_jadwal SET soal_postdate = '$today', ".
					"soal_jml = '$tmboh' ".
					"WHERE kd = '$skd'");
