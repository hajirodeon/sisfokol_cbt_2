...
if ($s == "excel")
	...
	$worksheet1->write_string(0,2,"JADWAL");
	$worksheet1->write_string(0,3,"SISWA");
	$worksheet1->write_string(0,4,"SOAL");
	$worksheet1->write_string(0,5,"JAWAB");


	//query	
	$qyukx = mysqli_query($koneksi, "SELECT * FROM siswa_soal ".
							"WHERE postdate LIKE '$tglku%' ".
	...
	do 
		...
		//detail soal
		$qyuk2 = mysqli_query($koneksi, "SELECT * FROM m_soal ".
								"WHERE kd = '$i_skd'");
		...
		$tableku = "siswa_soal_nilai";
		$qyuk2x = mysqli_query($koneksi, "SELECT * FROM $tableku ".
								"WHERE siswa_kd = '$i_swkd' ".
								"ORDER BY postdate DESC");
		$ryuk2x = mysqli_fetch_assoc($qyuk2x);
		...
		//update
		mysqli_query($koneksi, "UPDATE siswa_soal SET jadwal_kd = '$yuk2x_jkd' ".
						"WHERE kd = '$i_kd'");		
		...
		$worksheet1->write_string($dt_nox,2,$i_waktu);
		$worksheet1->write_string($dt_nox,3,$i_siswa);
		$worksheet1->write_string($dt_nox,4,$i_soalno);
		$worksheet1->write_string($dt_nox,5,$i_jawab);
		}
	...
	['.$tanggal.' '.$arrbln[$bulan].' '.$tahun.']. Siswa yang mengerjakan : <b>'.$tjos.'</b>]. 
	
	<a href="'.$filenya.'?s=excel&tglku='.$tglku.'" class="btn btn-success">EXPORT EXCEL >></a>
	...
	<td><strong><font color="'.$warnatext.'">JADWAL</font></strong></td>
